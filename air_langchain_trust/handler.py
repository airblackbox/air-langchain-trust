...
class AirTrustCallbackHandler(BaseCallbackHandler):  # type: ignore[misc]
    ...
    def __init__(self, config: AirTrustConfig | None = None) -> None:
        ...
        self.consent_gate = ConsentGate(
            self.config.consent_gate, self.ledger
        )
    ...
    def on_tool_start(
        ...
        **kwargs: Any,
        ...
        # 2. Check Gate policy (if configured)
        gate_decision = None
        if self.gate.is_configured:
            agent_id = (
                metadata.get("agent_id", "langchain-agent")
                if metadata
        ...
        if gate_decision:
            ...
            if decision == "blocked":
                ...
                logger.info(
                    "Gate PENDING: %s — sent to Slack",
                    tool_name,
                )
            elif decision == "auto_allowed":
                logger.debug(
                    "Gate AUTO-ALLOWED: %s", tool_name
                )

        # 3. Check local consent gate
        ...
        if self.config.audit_ledger.enabled:
            self.ledger.append(
                action="tool_call",
                tool_name=tool_name,
                risk_level=self.consent_gate.classify_risk(
                    tool_name
                ).value,
                consent_required=self.consent_gate.requires_consent(
                    tool_name
                ),
    ...
    def on_llm_start(
        ...
        **kwargs: Any,
        ...
        if (
            self.config.injection_detection.enabled
